...
export default function DynamicWeatherBackground({
  ...
  return (
    <div className="relative w-full overflow-hidden transition-all duration-700">
      ...
      {/* Dynamic Interactive Weather Ambient Indicator Header */}
      <div className="relative z-10 max-w-7xl mx-auto px-4 pt-2 pb-1">
        <div
          className={`rounded-2xl p-3.5 border transition-all duration-500 flex flex-col md:flex-row items-center justify-between gap-3 shadow-lg backdrop-blur-md ${
            isDark
              ? "bg-zinc-950/85 border-zinc-800/80 text-zinc-100"
              : "bg-white/85 border-[#E7E1D1] text-[#1F1B16]"
          }`}
        >
          {/* Main Status & Period Icon */}
          <div className="flex items-center gap-3 w-full md:w-auto">
            <div className="relative flex items-center justify-center w-11 h-11 rounded-2xl bg-gradient-to-br from-amber-500/20 via-sky-500/20 to-blue-600/20 border border-amber-500/30 text-2xl shrink-0 shadow-inner">
              <span>{conditionIcon}</span>
              {/* Pulsing live indicator light */}
              <span className="absolute -top-1 -right-1 w-3 h-3 rounded-full bg-emerald-500 border-2 border-white dark:border-zinc-950 animate-ping" />
              <span className="absolute -top-1 -right-1 w-3 h-3 rounded-full bg-emerald-500 border-2 border-white dark:border-zinc-950" />
            </div>

            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <span className="px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-wider bg-sky-500/15 text-sky-600 dark:text-sky-300 border border-sky-500/30">
                  Condição Projetada no Background
                </span>
                <span className="text-[10px] font-bold text-zinc-500 dark:text-zinc-400">
                  {cityName}
                </span>
                <span className="text-[10px] font-extrabold text-amber-500 dark:text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded-md border border-amber-500/20">
                  🕒 {effectiveTime.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })} (
                  {effectiveTime.toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit" })})
                </span>
              </div>

              <h2 className="text-sm font-black tracking-tight mt-0.5 flex items-center gap-2 flex-wrap">
                <span className="text-amber-500 dark:text-amber-400">{conditionLabel}</span>
                <span className="text-zinc-400 dark:text-zinc-500 font-normal">•</span>
                <span className="text-xs text-sky-600 dark:text-sky-300 font-bold">
                  {periodName}
                </span>
              </h2>
            </div>
          ...
          {/* Quick Metrics Badge for At-a-Glance Reality Check */}
          <div className="flex items-center gap-3 text-xs font-bold w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 pt-2 md:pt-0 border-zinc-200 dark:border-zinc-800">
            <div className="flex flex-col items-end">
              <span className="text-[9px] uppercase tracking-wider text-zinc-500 font-bold">
                Céu / Iluminação
              </span>
              <span className="text-xs font-black text-emerald-600 dark:text-emerald-400">
                {isSkyDarkened ? "☁️ Fechado / Opaco" : "🌤️ Iluminado / Claro"}
              </span>
            </div>
            ...
            <div className="flex flex-col items-end">
              ...
              <span className="text-xs font-black text-sky-600 dark:text-sky-400">
                {rainAmount > 0 ? `🌧️ ${rainAmount.toFixed(1)} mm/h` : "Sem Chuva"}
              </span>
            </div>
          </div>
        </div>
